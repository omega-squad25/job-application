  ...
  getSkills: async (req, res) => {
    ...
    try {
      ...
      if (!jobSeekerDetail) {
        return res
          .status(404)
          .json({ message: "JobSeekerDetail not found for this user" });
      }

      // Count total skills for pagination
      const totalCount = await jobSeekerDetail.countSkills();
      const totalPages = Math.ceil(totalCount / limit);

      return res.status(200).json({
        message: "Skills retrieved successfully",
        data: {
          skills: jobSeekerDetail.Skills.map((skill) => ({
            id: skill.id,
            name: skill.name,
  ...
  editSkill: async (req, res) => {
    ...
    try {
      await sequelize.transaction(async (t) => {
        ...
        if (!skillLink) {
          throw new Error(
            "Skill not found or not associated with your profile"
          );
        }
